  ...
  methods: {
    ...
    setCurrentStep: function(step) {
      ...
      window.history.pushState(
        "",
        "ignored param",
        window.location.pathname + "#/checkout/" + step
      );
    ...
    isCompleteStep: function(step) {
      return (
        this.STEPS.indexOf(step) < this.STEPS.indexOf(this.getCurrentStep())
      );
    },
  ...
  data: function() {
    return {
      ...
      paymentMethod: {
        ...
        expireMonth: "01",
        expireYear: "2040",
        paymentMethodId: "00000",
        paymentMethodTypeEnumId: undefined,
        postalContactMechId: "00000",
        telecomContactMechId: "0000000000",
        titleOnAccount: "placeholder"
      },
      shippingMethod: {},
      showProp65: "false",
      billingAddress: {},
  ...
  computed: {
    shippingPrice: function() {
      return this.shippingMethod &&
        this.shippingMethod.shippingTotal != undefined
        ? Number(this.shippingMethod.shippingTotal)
        : this.shippingItemPrice;
    }
  },
  methods: {
    notAddressSeleted: function() {
      return (
        ...
        this.addressOption == "" ||
        this.listShippingAddress == null ||
        this.listShippingAddress.length == 0
      );
    },
    notPaymentSeleted: function() {
      return (
        ...
        this.paymentOption == "" ||
        this.listPaymentMethods == null ||
        this.listPaymentMethods.length == 0
      );
    },
    getCustomerInfo: function() {
      CustomerService.getCustomerInfo(this.axiosConfig).then(
        function(data) {
    ...
    getCustomerShippingAddresses: function() {
      CustomerService.getShippingAddresses(this.axiosConfig).then(
        function(data) {
    ...
    onAddressUpserted: function(data) {
      ...
      this.addressOption =
        data.postalContactMechId + ":" + data.telecomContactMechId;
    ...
    getCartShippingOptions: function() {
      ProductService.getCartShippingOptions(this.axiosConfig).then(
        function(data) {
          ...
          for (var i in this.listShippingOptions) {
            if (!!this.listShippingOptions[i].shippingTotal) {
              this.listShippingOptions[i].shippingTotal = parseFloat(
                this.listShippingOptions[i].shippingTotal
              ).toFixed(2);
            }
          }
          ...
          var option = this.listShippingOptions
            ? this.listShippingOptions.find(function(item) {
                return item.shipmentMethodDescription == "Digital";
              })
            : 0;

          // Update the shipping option value
          if (!!option) {
            option.shippingTotal = parseFloat(this.shippingItemPrice).toFixed(
              2
            );
            this.shippingOption =
              option.carrierPartyId + ":" + option.shipmentMethodEnumId;
      ...
      ProductService.getCartInfo(this.axiosConfig).then(
        function(data) {
          if (data.postalAddress) {
            ...
            this.addressOption =
              data.postalAddress.contactMechId +
              ":" +
              data.postalAddress.telecomContactMechId;
            this.shippingAddressSelect = data.postalAddress;
            this.shippingAddressSelect.contactNumber =
              data.telecomNumber.contactNumber;
          } else if (this.listShippingAddress.length) {
            // Preselect first address
            this.addressOption =
              this.listShippingAddress[0].postalContactMechId +
              ":" +
              this.listShippingAddress[0].telecomContactMechId;
          }

          if (data.paymentInfoList && data.paymentInfoList.length) {
            this.paymentOption =
              data.paymentInfoList[0].payment.paymentMethodId;
            this.billingAddressOption =
              data.paymentInfoList[0].paymentMethod.postalContactMechId +
              ":" +
              data.paymentInfoList[0].paymentMethod.telecomContactMechId;
            this.selectBillingAddress(data.paymentInfoList[0]);
            for (var x in this.listPaymentMethods) {
              if (
                this.paymentOption ===
                this.listPaymentMethods[x].paymentMethodId
              ) {
    ...
    setShippingItemPrice: function() {
      ...
      var item = this.productsInCart.orderItemList
        ? this.productsInCart.orderItemList.find(function(item) {
            return item.itemTypeEnumId == "ItemShipping";
          })
        : 0;
    ...
    addCartBillingShipping: function() {
      ...
      ProductService.addCartBillingShipping(info, this.axiosConfig).then(
        function(data) {
    ...
    getCustomerPaymentMethods: function() {
      CustomerService.getPaymentMethods(this.axiosConfig).then(
        function(data) {
    ...
    placeCartOrder: function() {
      ...
      ProductService.placeCartOrder(data, this.axiosConfig)
        .then(
          function(data) {
            if (data.orderHeader != null) {
              this.$router.push({
                name: "successcheckout",
                params: { orderId: data.orderHeader.orderId }
              });
            } else {
            ...
            if (
              data.messages.includes("error") &&
              data.messages.includes("122")
            ) {
    ...
    applyPromotionCode: function() {
      var dataCode = {
        ...
        orderId: this.productsInCart.orderHeader.orderId
      };
      ProductService.addPromoCode(dataCode, this.axiosConfig).then(
        function(data) {
    ...
    deleteShippingAddress: function(contactMechId, contactMechPurposeId) {
      CustomerService.deleteShippingAddress(
        contactMechId,
    ...
    updateProductQuantity: function(item) {
      ...
      ProductService.updateProductQuantity(data, this.axiosConfig).then(
        function(data) {
          this.getCartInfo();
    ...
    deleteOrderProduct: function(item) {
      ProductService.deleteOrderProduct(
        item.orderId,
        item.orderItemSeqId,
        this.axiosConfig
      ).then(
    ...
    selectAddress: function(address) {
      ...
      this.shippingAddress.stateProvinceGeoId =
        address.postalAddress.stateProvinceGeoId;
    ...
    changeShippingAddress: function(data) {
      ...
      this.shippingAddressSelect.contactNumber =
        data.telecomNumber.contactNumber;
      this.postalAddressStateGeoSelected = {
        geoName: data.postalAddressStateGeo.geoName
      };
    },
    cleanShippingAddress: function() {
      this.shippingAddress = {};
      this.isUpdate = false;
    },
...
storeComps.CheckOutPageTemplate = getPlaceholderRoute(
  "template_client_checkout",
  "CheckOutPage"
);
...
storeComps.SuccessCheckOut = {
  ...
  methods: {
    ...
    getCustomerOrders: function() {
      CustomerService.getCustomerOrders(this.axiosConfig).then(
        function(data) {
    ...
    getCustomerOrderById: function() {
      CustomerService.getCustomerOrderById(
...
storeComps.SuccessCheckOutTemplate = getPlaceholderRoute(
  "template_client_checkoutSuccess",
  "SuccessCheckOut"
);

storeComps.CheckoutContactInfoTemplate = getPlaceholderRoute(
  "template_client_contactInfo",
  "contactInfo"
);
Vue.component("contact-info", storeComps.CheckoutContactInfoTemplate);

storeComps.CheckoutProp65Template = getPlaceholderRoute(
  "template_client_prop65",
  "prop65Warning"
);
Vue.component("prop65-warning", storeComps.CheckoutProp65Template);

storeComps.CheckoutNavbarTemplate = getPlaceholderRoute(
  "template_client_checkoutHeader",
  "CheckoutNavbar",
  storeComps.CheckoutNavbar.props
);
